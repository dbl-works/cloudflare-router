...
2. **Every value is checked at runtime.** TypeScript types do not protect a JavaScript consumer. The config, each route object and each rule must be a plain object with a normal or null prototype, and only own fields count. An unknown key on any of them throws. A sparse list throws. A key host must be a DNS name or an IPv4 literal. `spa` and `cors` must be `true` or `false`. `edgeCacheTtl` must be a whole number of seconds, 0 or more. A `basic` rule needs a non-empty username and password. An `ip` rule needs at least one address, and each entry must be one IPv4 or IPv6 address. Addresses compare in canonical form, so `2001:DB8::1` equals `2001:db8:0:0:0:0:0:1`.
